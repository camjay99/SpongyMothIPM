...
windows = load_daymet_forcing.create_window_grid(
   '/lustre/scratch5/cscholl/modis/2001_01_01.tif')
window = windows[300]
...
for year in range(2001, 2025):
  with rio.open(f'/lustre/scratch5/cscholl/landcover/{year}_01_01.tif',
                boundless=True,
...
for i in range(output_x):
  for j in range(output_y):
    ...
    for k in range(num_years):
      ...
      sample_year = [s[sample[0] == k] for s in sample]
      ...
      # Remove lead days
      tavg_sample = tavg_sample[31:, :]
      dayl_sample = dayl_sample[31:, :]
      cu_sample = cu_sample[31:, :]
      ...
      sos_sample = sos_year >= sos_year[(slice(None),) + (sample_year[1], sample_year[2])].reshape(1,-1)
